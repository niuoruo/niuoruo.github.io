...
给定单目视频作为输入，参数估计通常被表述为一个一般的非线性优化问题。为此，首先将所有未知量堆叠到参数向量$P$中。最近的最先进方法通常针对以下全部或部分参数进行优化：全局头部**旋转**$R$和**平移**$t$，**面部形状** $\alpha_i^{m_s}$ ，**反射率** $\beta_i^{m_r}$ ，**表情** $\sigma_i^{m_e}$ 和**光照** $l_i^{m_i}$ 。
